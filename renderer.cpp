...
void Renderer::MouseProcessing(int button)
{
	...
	if (scn->isPicked )
	{
		if (button == 1)
		{
			...
			scn->data().TranslateInSystem(scn->GetRotation(), 
				Eigen::Vector3d(xToMove, 0, 0));
			scn->data().TranslateInSystem(scn->GetRotation(),
				Eigen::Vector3d(0, yToMove, 0));
			scn->WhenTranslate();
		}
		else
		{
			...
			scn->data().RotateInSystem( Eigen::Vector3d(0, 0, 1), xrel / 100.0);
...
void Renderer::SetCameraTranslation(Eigen::Vector3f t) {
	//std::cout << "camera translation before: " << core().camera_translation << std::endl;
	core().camera_translation = *new Eigen::Vector3f(t);
	//std::cout << "camera translation after: " << core().camera_translation << std::endl;
}

void Renderer::SetCameraEye(Eigen::Vector3f e) {
	//std::cout << "camera eye before: " << core().camera_eye << std::endl;
	float base_zoom = core().camera_base_zoom;
	auto scale = Eigen::Scaling(core().camera_zoom * core().camera_base_zoom);
	Eigen::Vector3f eye = scale.inverse()*e;
	//eye << e(0), e(1), 1 + (e(2) - 1) * base_zoom;
	//= Eigen::Vector3f(0, 0, 1 + (1.6 - 1) * render->);
	core().camera_eye = *new Eigen::Vector3f(e);
	//std::cout << "camera eye after: " << core().camera_eye << std::endl;
}

void Renderer::centerCamera(Eigen::Vector3f nCenter) {
	core().camera_center = nCenter;
}

void Renderer::printCameraParameters() {
	std::cout << "camera center =	" << core().camera_center << std::endl;
	...
	std::cout << "base translation= " << core().camera_base_translation << std::endl;
	std::cout << "zoom= " << core().camera_zoom << std::endl;
	std::cout << "base zoom= " << core().camera_base_zoom << std::endl;
}
...
void Renderer::RotateCamera(float amtX, float amtY)
{
	...
	std::cout << "eye  " << core().camera_eye.transpose() << " center  " << core().camera_center.transpose() << std::endl;
	std::cout << "up  " << core().camera_up.transpose() << "  base translation  " << core().camera_base_translation.transpose() << std::endl;
	if (amtY != 0) {
	...
	if(amtX != 0) {
		...
		//core().camera_up = Eigen::Vector3f(1, 0, 0);
		Eigen::Matrix3f xMat;
		//xMat << cos(amtX), 0, sin(amtX), 0, 1, 0, -sin(amtX), 0, cos(amtX);
		xMat << 1, 0, 0, 0, cos(amtX), -sin(amtX), 0, sin(amtX), cos(amtX);
		core().camera_eye = xMat * core().camera_eye;
		//core().camera_up = Eigen::Vector3f(0, 1, 0);
	}
	std::cout << "after" << std::endl;
	std::cout << "eye " << core().camera_eye.transpose() << " center " << core().camera_center.transpose() << std::endl;
	std::cout << "up " << core().camera_up.transpose() << "  base translation " << core().camera_base_translation.transpose() << std::endl;
	std::cout << "translation: " << core().camera_translation.transpose() << std::endl;
